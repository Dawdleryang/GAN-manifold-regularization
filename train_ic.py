...
from ic_gan import discriminator, generator
import pandas as pd
import sys
...
flags = tf.app.flags
flags.DEFINE_integer("batch_size", 120, "batch size [50]")
flags.DEFINE_string('data_dir', './data/ic', 'data directory')
flags.DEFINE_string('logdir', './log/ic', 'log directory')
...
def read_data(csv_file):
    ...
    input = data.iloc[:,3:8]   # try differnt columns 5+some or all input column
    input = np.array(input)
    output = data.iloc[:,8]
    output = np.array(output)
    output[np.where(output=='pass')] = 0  #50 above
...
def main(_):
    ...
    #for IC Design
    csv_file = "/home/xlyang/kaggle/icdm/ICDesign/AnalogDataOpamp_v1.csv"
    input, output = read_data(csv_file)
    input = input.astype('float32')
    inds = rng_data.permutation(input.shape[0])
    input = input[inds]
    output = output[inds]

    nTr = int(0.8*input.shape[0])
    trainx = input[:nTr]
    trainy = output[:nTr]
    testx = input[nTr:]
    testy = output[nTr:]
    ...
    unl = tf.placeholder(tf.float32, [FLAGS.batch_size, 5], name='unlabeled_data_input_pl')
    is_training_pl = tf.placeholder(tf.bool, [], name='is_training_pl')
    inp = tf.placeholder(tf.float32, [FLAGS.batch_size, 5], name='labeled_data_input_pl')
    ...
    #gen_inp = tf.expand_dims(gen_inp, axis=-1)
    gen_inp_pert = generator(random_z_pert, is_training_pl, init=False, reuse=True)
    #gen_inp_pert = tf.expand_dims(gen_inp_pert, axis=-1)
    #import pdb; pdb.set_trace()
